...
class App:
    def __init__(self):
        ...
        Button(text='Pulsa', width=35, command=self.buttonCount, padx=15).grid(column=0, row=1, columnspan=2)

        self.entry = Entry()
        self.entry.grid(column=0, row=2)
        Button(text='Pulsa', command=self.buttonEntry).grid(column=1, row=2)
    ...
    def buttonEntry(self):
        self.text.configure(text=f'El texto contiene {self.entry.get()}')

    def buttonCombo(self):
        self.text.configure(text=f'El índice seleccionado es {self.combo.current()}')
